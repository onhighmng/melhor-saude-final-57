-- Update the get_admin_analytics function to include pillar data
...
AS $function$
  ...
  pillar_distribution_data AS (
    SELECT jsonb_agg(
      jsonb_build_object(
        ...
        'value', CASE 
          ...
          ELSE ROUND((booking_count::numeric / total_bookings::numeric) * 100, 1)
        END,
        'color', CASE 
    ...
    FROM pillar_stats
    CROSS JOIN (SELECT COUNT(*) as total_bookings FROM bookings) tb
  ...
  SELECT 
    ...
    (SELECT jsonb_build_object(
      'monthly', COALESCE(trends, '[]'::jsonb),
      'weekly', '[]'::jsonb,
      'overall', '[]'::jsonb
    ) FROM pillar_trends_data) as pillar_trends,
    (SELECT COALESCE(activity, '[]'::jsonb) FROM session_activity_data) as session_activity;
$function$