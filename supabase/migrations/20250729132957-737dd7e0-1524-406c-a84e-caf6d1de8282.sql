-- Update the get_admin_analytics function to include real session activity data
...
BEGIN
  ...
  -- Build session activity data (last 6 months)
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'period', activity_data.period,
      'agendadas', COALESCE(activity_data.agendadas, 0),
      'comparecidas', COALESCE(activity_data.comparecidas, 0),
      'completadas', COALESCE(activity_data.completadas, 0)
    ) ORDER BY activity_data.month_start
  ), '[]'::jsonb) INTO session_activity_data
  FROM (
    SELECT 
      DATE_TRUNC('month', b.created_at) as month_start,
      TO_CHAR(b.created_at, 'Mon YYYY') as period,
      COUNT(*) as agendadas,
      COUNT(CASE WHEN b.status IN ('confirmed', 'completed') THEN 1 END) as comparecidas,
      COUNT(CASE WHEN b.status = 'completed' THEN 1 END) as completadas
    FROM bookings b
    WHERE b.created_at >= NOW() - INTERVAL '6 months'
    GROUP BY DATE_TRUNC('month', b.created_at), TO_CHAR(b.created_at, 'Mon YYYY')
  ) activity_data;
  ...
  result := jsonb_build_object(
    ...
    'session_activity', COALESCE(session_activity_data, '[]'::jsonb)
  );
