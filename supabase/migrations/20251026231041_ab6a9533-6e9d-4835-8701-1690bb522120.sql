...
-- Drop and recreate specialist_analytics view
DROP VIEW IF EXISTS specialist_analytics CASCADE;

CREATE VIEW specialist_analytics AS
SELECT 
  pillar,
...
-- Drop and recreate user_profile_with_roles view
DROP VIEW IF EXISTS user_profile_with_roles CASCADE;

CREATE VIEW user_profile_with_roles AS
SELECT 
  p.id,
  p.name,
  p.email,
