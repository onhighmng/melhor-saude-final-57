...
const PreferencesAndExpectations: React.FC<PreferencesAndExpectationsProps> = ({ preferencesData, onPreferencesChange, onNext, onBack }) => {
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      <Button variant="ghost" onClick={onBack}>Voltar</Button>
      <div className="text-center"><h1 className="text-4xl font-serif font-bold mb-4">Preferências</h1></div>
      <Card className="p-6 space-y-6">
        <div className="space-y-4">
          <label className="text-base font-semibold">Formato preferido?</label>
          <div className="grid grid-cols-2 gap-3">
            {['in-person', 'online', 'phone'].map(val => (
              <button key={val} onClick={() => onPreferencesChange({...preferencesData, sessionFormat: val})}
                ...
                {val === 'in-person' ? 'Presencial' : val === 'online' ? 'Online' : 'Telefone'}
              </button>
            ))}
          </div>
        </div>
        <Textarea value={preferencesData.successDescription} onChange={(e) => onPreferencesChange({...preferencesData, successDescription: e.target.value})} 
          placeholder="Resultado esperado?" className="min-h-[120px]" />
      </Card>
